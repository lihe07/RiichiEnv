...
from riichienv import ReplayGame, AgariCalculator, Conditions
    ...
    for i, log_path in enumerate(sorted(list(log_dir.glob("251214-*.json.gz")))):
        ...
        for kyoku in game.take_kyokus():
            for ctx in kyoku.take_agari_contexts():
                ...
                t0 = time.time()
                cond_py = Conditions()
                for attr in ["tsumo", "riichi", "double_riichi", "ippatsu", "haitei", "houtei", "rinshan", "chankan", "tsumo_first_turn", "player_wind", "round_wind", "kyoutaku", "tsumi"]:
                ...
                if res_m.error:
                    ...
                    print(f"  Conditions: riichi={ctx.conditions.riichi}, tsumo={ctx.conditions.tsumo}, p_wind={ctx.conditions.player_wind}, r_wind={ctx.conditions.round_wind}")
